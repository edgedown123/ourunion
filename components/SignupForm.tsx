...
const SignupForm: React.FC<SignupFormProps> = ({ onGoHome, onAddMember, onRemoveMember, existingMembers }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'loginId') { setIsIdChecked(false); setIdError(''); }
  };
  ...
  const handleCheckId = () => {
    ...
    if (exists) { setIdError('이미 사용 중인 아이디입니다.'); setIsIdChecked(false); }
    ...
    return (
      <div className="max-w-2xl mx-auto py-20 px-4 text-center animate-fadeIn">
        <div className="bg-sky-50 rounded-3xl p-12 border border-sky-100 shadow-sm">
          ...
          <p className="text-lg text-gray-600 mb-8">우리노동조합 가입 신청이 정상적으로 접수되었습니다.<br/>지금부터 입력하신 아이디로 로그인이 가능합니다.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-4 animate-fadeIn">
      {isWithdrawMode ? (
        <div className="bg-white border rounded-3xl p-10 shadow-sm space-y-8 animate-fadeIn">
          ...
          <form onSubmit={handleWithdrawSubmit} className="space-y-4">
            <div>
              ...
              <input required type="text" name="loginId" value={withdrawData.loginId} onChange={handleWithdrawChange} className="w-full border-2 border-gray-100 rounded-xl p-3 outline-none focus:border-red-400 transition-all" placeholder="아이디 입력" />
            </div>
            <div>
              <label className="block text-xs font-black text-gray-500 mb-2 uppercase tracking-widest">비밀번호</label>
              <input required type="password" name="password" value={withdrawData.password} onChange={handleWithdrawChange} className="w-full border-2 border-gray-100 rounded-xl p-3 outline-none focus:border-red-400 transition-all" placeholder="비밀번호 입력" />
            </div>
            <div className="pt-4 flex space-x-2">
              <button type="button" onClick={() => setIsWithdrawMode(false)} className="flex-1 py-4 bg-gray-100 text-gray-600 rounded-xl font-bold hover:bg-gray-200 transition-all">취소</button>
              <button type="submit" className="flex-1 py-4 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 shadow-lg shadow-red-50 transition-all">탈퇴 완료</button>
            </div>
          </form>
        </div>
      ) : (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <input required type="text" name="loginId" value={formData.loginId} onChange={handleChange} className={`flex-1 border-gray-300 rounded-lg p-3 border focus:ring-2 focus:ring-sky-primary outline-none transition-all ${isIdChecked ? 'bg-emerald-50 border-emerald-200' : ''}`} placeholder="" />
                  <button type="button" onClick={handleCheckId} className="px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-bold hover:bg-black whitespace-nowrap">중복확인</button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input required type="password" name="passwordConfirm" value={formData.passwordConfirm} onChange={handleChange} className={`w-full border-gray-300 rounded-lg p-3 border focus:ring-2 focus:ring-sky-primary outline-none ${isPasswordMismatch ? 'border-red-300 bg-red-50' : ''}`} />
                  {isPasswordMismatch && <p className="text-red-500 text-xs mt-1 font-medium">비밀번호가 일치하지 않습니다.</p>}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full border-gray-300 rounded-lg p-3 border focus:ring-2 focus:ring-sky-primary outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">생년월일 (6자리)</label>
                  <input required type="text" name="birthDate" value={formData.birthDate} onChange={handleChange} maxLength={6} pattern="\d{6}" className="w-full border-gray-300 rounded-lg p-3 border focus:ring-2 focus:ring-sky-primary outline-none" placeholder="예: 650718" />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <input required type="email" name="email" value={formData.email} onChange={handleChange} className="w-full border-gray-300 rounded-lg p-3 border focus:ring-2 focus:ring-sky-primary outline-none" placeholder="example@email.com" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">소속 차고지</label>
                <input required type="text" name="garage" value={formData.garage} onChange={handleChange} className="w-full border-gray-300 rounded-lg p-3 border focus:ring-2 focus:ring-sky-primary outline-none" placeholder="예: 진관, 도봉, 송파" />
              </div>
            </div>
