            ...
            <div className="mt-20 p-8 md:p-10 bg-gray-50/50 rounded-[2.5rem] border-2 border-dashed border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedPost.attachments.map((file, idx) => (
                  <div
                    key={idx}
                    className="bg-white p-5 rounded-2xl border flex flex-col gap-3 md:flex-row md:items-center md:justify-between shadow-sm hover:border-sky-primary transition-all group"
                  >
                    <div className="flex items-start min-w-0 gap-4">
                      <i className={`fas ${file.type.startsWith('image/') ? 'fa-file-image' : 'fa-file-alt'} text-gray-300 mt-0.5 text-xl`}></i>
                      <span className="text-sm font-bold text-gray-700 whitespace-normal break-all leading-snug">
                        {file.name}
                      </span>
                    </div>
                    <a
                      href={file.data}
                      download={file.name}
                      className="self-end md:self-auto px-5 py-2 bg-sky-primary text-white text-[11px] font-black rounded-xl shadow-md hover:opacity-90 active:scale-95 transition-all"
                    >
                      다운로드
                    </a>
                  </div>
